...
create index if not exists billing_payments_provider_checkout_lookup_idx
  on public.billing_payments (provider, provider_checkout_id)
  where provider_checkout_id is not null;

create index if not exists billing_payments_provider_payment_lookup_idx
...
declare
  ...
  old_status text;
  effective_organization_id uuid := p_organization_id;
...
begin
  ...
  if lifecycle_status = 'paid' then
    if normalized_checkout_id is null
...
create or replace function public.replay_billing_webhook_event(
  p_event_uuid uuid,
...
begin
  ...
  where id = p_event_uuid and provider = 'paymongo'
  for update;

  if not found then
    raise exception 'PayMongo billing event not found.';
  end if;
  if event_row.organization_id is null then
    raise exception 'The billing event is not assigned to an organization.';
  end if;
  if event_row.status not in ('failed', 'ignored') then
    raise exception 'Only failed or ignored billing events can be replayed.';
  end if;

  select member.role into role_text
  from public.organization_members member
  where member.organization_id = event_row.organization_id
  ...
  update public.billing_payment_events
  set status = 'received', next_retry_at = now(), dead_lettered_at = null,
      ignored_reason = null, error_message = null,
      replayed_at = now(), replayed_by = p_actor_user_id, updated_at = now()
  where id = event_row.id;

  return jsonb_build_object(
    'ok', true,
    'provider_event_id', event_row.provider_event_id,
    'signature_timestamp', event_row.signature_timestamp,
...
revoke all on function public.replay_billing_webhook_event(uuid, uuid)
  from public, anon, authenticated;
...
grant execute on function public.replay_billing_webhook_event(uuid, uuid)
  to service_role;

comment on function public.process_paymongo_lifecycle_event(
  text, text, boolean, timestamptz, text, text, uuid, text, text, text,
  integer, text, text, text, text, jsonb
) is 'Validates and applies PayMongo lifecycle events with amount, currency, tenant, checkout, stale-event, refund, and idempotency protection.';
