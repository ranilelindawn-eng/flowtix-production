-- Flowtix AI usage reservation repair
--
-- Fixes production error:
--   ERROR 42702: column reference "metric" is ambiguous
--
-- Root cause:
-- consume_organization_usage() RETURNS TABLE exposes an output variable named
-- "metric". A later billing/trial migration reintroduced:
--
--   ON CONFLICT (organization_id, metric, period_start) DO NOTHING
--
-- Inside PL/pgSQL, "metric" can refer either to the RETURNS TABLE output
-- variable or to the organization_usage_counters column.
--
-- This migration preserves the current subscription/trial enforcement logic
-- and changes only the ambiguous conflict target to the existing primary-key
-- constraint name.
