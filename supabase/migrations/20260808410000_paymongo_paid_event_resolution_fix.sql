...
begin
  ...
  if lifecycle_status = 'paid' then
    ...
    -- PayMongo payment.paid resources identify the payment directly but may not
    -- include the parent checkout session ID. Once tenant and pending plan
    -- metadata have been validated, resolve the checkout from the locked
    -- subscription row so the event can still be matched to the reserved
    -- pending billing payment without trusting a client-supplied checkout ID.
