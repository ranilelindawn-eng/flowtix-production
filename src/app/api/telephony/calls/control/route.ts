...
export async function POST(request: Request) {
  ...
  if (body.action === 'start-recording') await client.calls(sid).recordings.create({ recordingChannels: 'dual', recordingStatusCallback: `${config.publicUrl}/api/telephony/recording?organizationId=${encodeURIComponent(organization.organization_id)}` })
