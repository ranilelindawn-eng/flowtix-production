...
function nullableDate(value: string, timeZone: string): string | null {
  if (!value) return null
  const parsed = organizationLocalDateTimeToUtc(value, timeZone)
  if (!parsed) throw new Error('A valid date and time is required.')
  return parsed
}
...
export async function createAdvancedTask(formData: FormData) {
  ...
  const startAt = nullableDate(text(formData, 'startAt'), timeZone)
  const dueAt = nullableDate(text(formData, 'dueAt'), timeZone)
  const reminderAt = nullableDate(text(formData, 'reminderAt'), timeZone)
...
export async function updateAdvancedTask(formData: FormData) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      start_at: startAt,
      due_at: dueAt,
      reminder_at: nullableDate(text(formData, 'reminderAt'), timeZone),
