...
function TranscriptCard({
  ...
  return (
    <Link
      ...
      aria-label={`Open ${getLanguageLabel(transcript.language)} transcript created ${formatDate(transcript.created_at, timeZone)}`}
      className="group flex h-full flex-col rounded-2xl border border-slate-800 bg-slate-900/70 p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-blue-500/60 hover:bg-slate-900 hover:shadow-xl hover:shadow-blue-950/20 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-950"
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-950 px-3 py-1.5 text-xs font-semibold text-slate-300 transition group-hover:border-blue-500 group-hover:text-white">
          Open
          <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5">
    ...
    <div className="space-y-6 lg:relative lg:left-1/2 lg:w-[calc(100vw-328px)] lg:max-w-[1800px] lg:-translate-x-1/2">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        <form
          ...
          method="get"
          className="grid gap-3 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)_auto_auto]"
        >
          <div>
          ...
          {hasFilters ? (
            ...
            <div className="mx-auto mt-7 grid max-w-2xl gap-3 sm:grid-cols-2">
              <Link
                ...
                className="group rounded-2xl border border-slate-700 bg-slate-900 px-5 py-4 text-left transition hover:-translate-y-0.5 hover:border-blue-500/60 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <span className="block text-sm font-semibold text-white">Open recordings</span>
                <span className="mt-1 block text-xs leading-5 text-slate-400">Upload or review the source call recording used for transcription.</span>
              </Link>

              <Link
                href="/dashboard/transcripts/new"
                className="group rounded-2xl border border-blue-500/40 bg-blue-600/10 px-5 py-4 text-left transition hover:-translate-y-0.5 hover:border-blue-400 hover:bg-blue-600/15 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <span className="block text-sm font-semibold text-white">Create transcript</span>
                <span className="mt-1 block text-xs leading-5 text-slate-400">Create a transcript from an existing recording in this workspace.</span>
              </Link>
            </div>
          )}
        </div>
