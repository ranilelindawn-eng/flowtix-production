...
export default async function PhoneNumbersPage() {
  ...
  return <div className="space-y-8">
    <div><h1 className="text-3xl font-bold">Phone Numbers</h1><p className="mt-2 text-muted-foreground">Manage SignalWire numbers, capabilities, default caller ID, and inbound Ring Group or Queue routing.</p></div>
    {manageable ? <form action={addPhoneNumber} className="rounded-xl border border-border bg-card p-6">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3"><input name="friendly_name" placeholder="Sales line" className="rounded-lg border bg-background px-3 py-2"/><input name="phone_number" required placeholder="+15551234567" className="rounded-lg border bg-background px-3 py-2"/><input type="hidden" name="provider" value="signalwire"/><div className="rounded-lg border bg-background px-3 py-2 text-sm">SignalWire</div></div>
    ...
    <div className="grid gap-4">{(numbers ?? []).map((n) => <article key={n.id} className="rounded-xl border border-border bg-card p-5">
      <div className="flex flex-col gap-5">
        ...
        <div className="rounded-xl border border-border bg-background/40 p-4"><h3 className="font-medium">Inbound routing</h3><p className="mt-1 text-sm text-muted-foreground">SignalWire numbers use the same Flowtix Ring Group/Queue routing model.</p>
