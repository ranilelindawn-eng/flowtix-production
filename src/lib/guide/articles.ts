...
export const guideArticles: GuideArticle[] = [
  ...
  {
    slug: 'email-sms', title: 'Conversations', category: 'Sales & Automation', summary: 'Read, assign, and reply to tenant-scoped customer Email and SMS conversations from one inbox.', moduleHref: '/dashboard/communications', moduleLabel: 'Open Conversations', pathnamePrefixes: ['/dashboard/communications'],
    prerequisites: ['A supported communication provider/integration must be configured before live sending.', 'Connect Gmail and configure the Gmail Pub/Sub inbox watch to receive email replies in Flowtix.', 'Inbound SMS uses the organization Business SMS Number and its existing SignalWire webhook.'],
    steps: [
      { title: 'Choose the right inbox view', instructions: ['Use Inbox, Unread, My Inbox, Unassigned, or Closed to focus the queue.', 'Filter by Email or SMS and search by customer/conversation details.'] },
      { title: 'Read and assign the conversation', instructions: ['Open a customer thread to review inbound and outbound Email/SMS history in chronological order.', 'Owners, admins, and managers can assign conversations according to their permissions.'] },
      { title: 'Reply from Flowtix', instructions: ['Choose Email or SMS in the reply composer.', 'Review the destination and message before sending; Flowtix uses the organization-owned provider connection and records delivery status.'] },
      { title: 'Verify the result', instructions: ['Confirm the outbound reply appears in the same thread and advances through queued/sent/delivered or failed status.', 'Open the related contact to verify recent conversations and timeline activity are linked correctly.'] },
    ], successChecks: ['Inbound customer replies appear only in the correct organization.', 'Unread state is independent for each team member.', 'Authorized members can read/reply while assignment and role restrictions remain enforced.', 'SMS opt-out handling remains active.'], troubleshooting: ['For missing email replies, check the Gmail integration and Pub/Sub watch status shown in Conversations.', 'For missing SMS replies, verify the active Business SMS Number and SignalWire inbound configuration.', 'If sending fails, inspect the provider status and the existing durable communications job before retrying.'], related: ['templates','snippets','contacts','settings']
  },
  {
  ...
  {
    slug: 'dialer', title: 'Dialer', category: 'Calling & Telephony', summary: 'Use the Flowtix-managed browser softphone to make controlled outbound calls.', moduleHref: '/dashboard/dialer', moduleLabel: 'Open Dialer', pathnamePrefixes: ['/dashboard/dialer'],
    prerequisites: ['Your plan and role must allow outbound calling.', 'Your browser must allow microphone access.'],
    steps: [
      { title: 'Bring the softphone online', instructions: ['Open Dialer and wait for Softphone online.', 'Flowtix assigns the outbound calling identity automatically; subscribers do not import or select a carrier number.', 'If it does not come online, use Reconnect. If the message persists, contact Flowtix support.'] },
      { title: 'Select the contact or destination', instructions: ['Choose one of your assigned CRM contacts or enter the destination directly.', 'Use E.164 format such as +12025550123 when entering a number manually.'] },
      { title: 'Place a controlled call', instructions: ['Press the green call button once.', 'Watch the call state and use Live Calls for in-progress visibility.', 'Business SMS Number registration is separate and is only required for SMS automation, not outbound calling.'] },
    ...
    ], successChecks: ['Softphone shows online without subscriber telephony setup.', 'Flowtix places the outbound call using the platform-managed calling identity.', 'Flowtix creates and updates the call lifecycle.', 'The completed call is visible in Calls.'], troubleshooting: ['If Flowtix calling infrastructure is unavailable, reconnect once and then contact Flowtix support rather than importing a personal/provider number.', 'Destination restrictions or carrier account limits can still cause a provider-side call rejection.'], related: ['live-calls','organization','calls']
  },
  {
