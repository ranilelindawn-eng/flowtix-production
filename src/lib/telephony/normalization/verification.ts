...
export async function verifyProviderCallWebhook(input: {
  ...
  const connection = await getOrganizationProviderConnection<Record<string, unknown>>(input.organizationId, input.provider)
  ...
  if (input.provider === 'twilio' || input.provider === 'signalwire') {
    ...
    const signature = input.headers.get('x-twilio-signature') || input.headers.get('x-signalwire-signature') || ''
  ...
  if (input.provider === 'telnyx') {
    const publicKey = String(connection.config.public_key ?? connection.credentials.publicKey ?? '')
    const signature = input.headers.get('telnyx-signature-ed25519') || ''
    const timestamp = input.headers.get('telnyx-timestamp') || ''
    if (!publicKey || !signature || !timestamp) return false
    const key = publicKey.includes('BEGIN PUBLIC KEY') ? publicKey : `-----BEGIN PUBLIC KEY-----\n${publicKey}\n-----END PUBLIC KEY-----`
    try { return verify(null, Buffer.from(`${timestamp}|${input.rawBody}`), key, Buffer.from(signature, 'base64')) } catch { return false }
  }
  ...
  const digest = createHmac('sha256', authToken).update(`${input.requestUrl}${nonce}`).digest('base64')
  return signature.split(',').some((candidate) => safeEqual(candidate.trim(), digest))
}
