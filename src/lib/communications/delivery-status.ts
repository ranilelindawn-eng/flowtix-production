...
export function expectedCallbackToken(
...
export function validateCallbackToken(input: {
  provider: ConfiguredTelephonyProviderName
  messageId: string
  token: string
}) {
  return safeEqual(
    expectedCallbackToken(input.provider, input.messageId),
    input.token,
  )
}
...
export function normalizeFormDeliveryEvent(
  provider: Exclude<DeliveryProvider, 'telnyx' | 'resend'>,
  ...
  if (!providerMessageId) {
    throw new Error('The provider message ID is missing.')
  }
  ...
  const eventAt = eventTime(
    form.get('Timestamp') ||
      form.get('EventTimestamp') ||
      form.get('message_time'),
  )
  ...
  return {
    ...
    metadata: {
      providerStatus,
      from: form.get('From') || form.get('FromNumber'),
      to: form.get('To') || form.get('ToNumber'),
    },
  }
}

export function normalizeTelnyxDeliveryEvent(
  ...
  const to = Array.isArray(payload.to)
    ? payload.to[0] as Record<string, unknown> | undefined
    : undefined
  ...
  if (!providerMessageId) {
    throw new Error('The Telnyx message ID is missing.')
  }
...
export function normalizeResendDeliveryEvent(
  ...
  if (!type || !providerMessageId) {
    throw new Error('The Resend delivery event is invalid.')
  }
  ...
  return {
    ...
    errorCode:
      type === 'email.bounced'
        ? stringValue(bounce?.type) || 'BOUNCED'
        : type === 'email.complained'
          ? 'COMPLAINED'
          : type === 'email.failed'
            ? 'FAILED'
...
export async function validateTwilioCompatibleSignature(input: {
  requestUrl: string
  signature: string
  form: URLSearchParams
  ...
  const token =
    input.provider === 'twilio'
      ? stringValue(connection.credentials.authToken)
      : stringValue(connection.credentials.apiToken)
  ...
  return twilio.validateRequest(
    ...
    input.signature,
    input.requestUrl,
    Object.fromEntries(input.form.entries()),
  )
}

function telnyxPublicKey() {
  return process.env.TELNYX_PUBLIC_KEY?.trim() || null
}
...
export function validateTelnyxSignature(input: {
  ...
  const configured = telnyxPublicKey()
...
export function validateResendSignature(input: {
  rawBody: string
  eventId: string
  timestamp: string
  ...
  const keyValue = secret.startsWith('whsec_')
    ? secret.slice(6)
    : secret
...
export async function findCommunicationMessage(input: {
  ...
  let query = client
    ...
    .select('id,organization_id,provider,provider_message_id')
  ...
  if (error) {
    throw new Error(
      `Unable to find communication message: ${error.message}`,
    )
  }
...
export async function applyDeliveryEvent(input: {
  ...
  const { data, error } = await client.rpc(
    'apply_communication_delivery_event',
    {
      ...
      p_provider_status:
        input.event.metadata.providerStatus ??
        input.event.metadata.type ??
        input.event.status,
  ...
  if (error) {
    throw new Error(
      `Unable to apply communication delivery event: ${error.message}`,
    )
  }
